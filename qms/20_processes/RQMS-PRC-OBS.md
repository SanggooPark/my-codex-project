...
| --- | --- |
| 문서번호 | RQMS-PRC-OBS |
| 영문명 | Obsolescence management process |
| 근거 조항 | ISO 22163:2023 8.10 |
| 구분 | 필수 프로세스 (Annex A.1) |
| 프로세스 오너 | 기술개발 책임자 (5.3.1 b, 5.3.2) |
| 상위 프로세스 | 없음 (최상위) |
| 하위 프로세스 | 없음 |
| 구현 모듈 | `rqms.services.obsolescence` |
| 프로세스 검토 주기 | 12개월 (9.4) |

## 1. 목적 및 적용범위

본 프로세스는 ISO 22163:2023 8.10 의 요구사항을 충족하기 위해 단산(진부화) 관리 프로세스를 수립·실행·유지한다.
...
| 8.10 | shall | ISO 22163 추가 | 단산(진부화) 관리 프로세스(리스크평가·관리계획·고객 의사소통) |

## 4. 강제 통제

| 통제 | 유형 | 중요도 | 내용 | 근거 조항 |
| --- | --- | --- | --- | --- |
| CTL-030 | audit | major | 공급 제품의 단산 리스크 평가 및 단산 관리계획 정기 검토 | 8.10 |
...
| --- | --- | --- | --- | --- | --- | --- |
| PI-OBS 단산 리스크 완화율 | PI | 완화조치 완료 단산 리스크 수 / 식별 단산 리스크 수 x 100 | 90.0% | quarterly | 기술개발 책임자 | 기술개발 책임자 |
...
| 증거 | 저장 위치 |
| --- | --- |
| 단산 관리계획 | `obsolescence_plan` |
| 단산 리스크 기록 | `obsolescence_risk` |
