# DND — 제품·서비스 설계 및 개발 프로세스

> 이 문서는 `tools/generate_qms_docs.py` 가 `src/rqms/data` 레지스트리에서 생성합니다.
> 직접 수정하지 말고 레지스트리를 수정한 뒤 재생성하십시오.

| 항목 | 내용 |
| --- | --- |
| 문서번호 | RQMS-PRC-DND |
| 영문명 | Design and development process |
| 근거 조항 | ISO 22163:2023 8.3.1 |
| 구분 | 필수 프로세스 (Annex A.1) |
| 프로세스 오너 | 기술개발 책임자 (5.3.1 b, 5.3.2) |
| 상위 프로세스 | 없음 (최상위) |
| 하위 프로세스 | 없음 |
| 구현 모듈 | `rqms.services.design` |
| 프로세스 검토 주기 | 12개월 (9.4) |

## 1. 목적 및 적용범위

본 프로세스는 ISO 22163:2023 8.3.1 의 요구사항을 충족하기 위해 제품·서비스 설계 및 개발 프로세스를 수립·실행·유지한다.

## 2. 프로세스 정의 (4.4.1 a~e)

프로세스의 입력·출력, 순서 및 상호작용, 판정기준과 성과지표, 필요자원, 책임과 권한은 RQMS 데이터베이스의 `process_instance` 레코드에 등록되어 있으며, 등록 없이는 프로세스를 운영할 수 없다(CTL-038, CTL-039).

| 4.4.1 항목 | 등록 위치 |
| --- | --- |
| a) 입력·출력 | `process_instance.inputs` / `.outputs` |
| b) 순서 및 상호작용 | `process_instance.sequence_note` |
| c) 판정기준·측정·성과지표 | `process_instance.criteria` 및 아래 5장 성과지표 |
| d) 필요 자원 | `process_instance.resources` |
| e) 책임 및 권한 | `process_instance.owner_id` (프로세스 오너) |
| 4.4.3 f) 리스크기반 통제기준 | `process_instance.risk_criteria` |

## 3. 관련 요구사항

| 조항 | 의무 | 출처 | 요구사항 |
| --- | --- | --- | --- |
| 8.3.1 | shall | ISO 9001:2015 | 설계 및 개발 프로세스 |
| 8.3.1.1 | shall | ISO 22163 추가 | 신기술 리스크 평가, 제품 아키텍처, RAMS/LCC, IEC 62278 적합 |
| 8.3.2 | shall | ISO 9001:2015 | 설계 및 개발 기획 |
| 8.3.2.1.1 | shall | ISO 22163 추가 | 단계 목표·아키텍처·형상관리·계층별 설계검토/검증/유효성확인 |
| 8.3.2.1.2 | should | ISO 22163 추가 | 단계별 품질보증 기법·기술요구사항·운용성숙도 통제 방법 |
| 8.3.3 | shall | ISO 9001:2015 | 설계 및 개발 입력 |
| 8.3.3.1.1 | shall | ISO 22163 추가 | 8.2.2 요구사항 및 식별·추적성 고려 |
| 8.3.3.1.2 | should | ISO 22163 추가 | 생산·routine test 요구사항 및 보존 요구사항 고려 |
| 8.3.4 | shall | ISO 9001:2015 | 설계 및 개발 관리 |
| 8.3.4.1 | should | ISO 22163 추가 | 기능분해·운용성숙도·품질보증 기법 고려 |
| 8.3.4.2 | shall | ISO 22163 추가 | 설계검토(다음 단계 진행 승인기준·필수 참석자·의사결정 권한) |
| 8.3.4.3 | shall | ISO 22163 추가 | 설계검증(기술요구사항 검증) |
| 8.3.4.4 | shall | ISO 22163 추가 | 설계 유효성확인(인도/시운전 종료 전 완료 또는 고객 합의 관리계획) |
| 8.3.4.5 | shall | ISO 22163 추가 | 검증·유효성확인 시험 요구사항(재현성·형상기준선·합격기준) |
| 8.3.5 | shall | ISO 9001:2015 | 설계 및 개발 출력 |
| 8.3.5.1.1 | shall | ISO 22163 추가 | 출력의 출시 전 검증·승인, 생산입력 대비 검증, 관련 문서·교육 포함 |
| 8.3.5.1.2 | should | ISO 22163 추가 | 입력-출력 추적성, 설계승인 권한·합격기준, 에스컬레이션 규칙 |
| 8.3.6 | shall | ISO 9001:2015 | 설계 및 개발 변경 |
| 8.5.1.1.3 | shall | ISO 22163 추가 | 생산공정 검증(설계출력 대비 입력 완전성·설비능력·공정FMEA) |

## 4. 강제 통제

| 통제 | 유형 | 중요도 | 내용 | 근거 조항 |
| --- | --- | --- | --- | --- |
| CTL-009 | invariant | major | 설계 출력은 검증·승인 후에만 출시 | 8.3.5, 8.3.5.1.1 |
| CTL-010 | invariant | major | 설계 유효성확인 미완료 품목의 인도 금지(고객 합의 관리계획이 있는 경우 예외) | 8.3.4.4 |
| CTL-027 | invariant | major | 변경은 영향분석·승인 후에만 실행하며 실행 후 검증 필수 | 6.3, 8.1.4.2, 8.2.4, 8.3.6, 8.5.6 |
| CTL-046 | audit | major | 요구사항은 기술규격으로 문서화되고 RAMS/LCC·단산·중요특성이 반영됨 | 8.2.2.1.1, 8.2.5, 8.3.4.3 |
| CTL-047 | invariant | major | 검증·유효성확인 시험은 시험계획(목적·조건·합격기준)과 형상기준선 기록 필요 | 8.3.2.1.1, 8.3.4.5 |
| CTL-050 | invariant | major | 승인된 생산데이터가 없으면 생산 착수 금지(관리된 조건) | 8.5.1.1.2, 8.5.1.1.3, 8.5.1.2.1 |
| CTL-057 | audit | major | 안전관련 제품은 적용 안전표준 식별 및 안전 케이스 보유 | 8.3.1.1, 8.8.3 |
| CTL-058 | invariant | minor | 설계검토는 승인기준과 필수 참석자(의사결정 권한) 요건 충족 | 8.3.4.2 |
| CTL-060 | invariant | major | 설계입력은 완전·명확해야 하며 상충 입력은 해결 후 확정 | 8.3.3, 8.3.3.1.1 |

`invariant` 통제는 업무 처리 시점에 시스템이 강제하며, 위반 시 해당 처리가 거부되고 `control_violation_log` 에 기록된다. `audit` 통제는 `python -m rqms check` 로 상태를 점검한다.

## 5. 성과지표 (4.4.1 c, 9.1.1.1)

| 지표 | 구분 | 산식 | 목표 | 주기 | 보고 대상 | 조치 책임 |
| --- | --- | --- | --- | --- | --- | --- |
| PI-REQ 요구사항 조항별 검토 완료율 | PI | 검토 완료 요구사항 수 / 등록 요구사항 수 x 100 | 100.0% | monthly | 기술개발 책임자, 사업관리(PMO) 책임자 | 기술개발 책임자 |

목표 미달 시 KPI 는 시정조치가 필수이며(9.1.3.1), 그 외 PI 는 프로세스 검토에서 조치 필요성을 판단한다.

## 6. 문서화된 정보 (7.5)

| 증거 | 저장 위치 |
| --- | --- |
| 변경요청 | `change_request` |
| 설계 입력 | `design` (조건: `inputs_complete = 1`) |
| 설계 출력 | `design` (조건: `output_released_on IS NOT NULL`) |
| 설계·개발 계획 | `design` (조건: `plan_doc_id IS NOT NULL`) |
| 설계검토 기록 | `design_review` |
| 생산공정 검증 | `production_order` (조건: `process_verified_on IS NOT NULL`) |
| 안전 케이스 | `safety_record` (조건: `safety_case_doc_id IS NOT NULL`) |
| 시험계획 | `design_test` (조건: `plan_doc_id IS NOT NULL`) |
| 시험 기록 | `design_test` (조건: `performed_on IS NOT NULL`) |
| 유효성확인 기록 | `design` (조건: `validated_on IS NOT NULL`) |
| 검증 기록 | `design` (조건: `verified_on IS NOT NULL`) |

## 7. 프로세스 검토 (9.4)

본 프로세스는 12개월 주기로 프로세스 오너가 주재하여 검토한다. 검토는 9.4 a)~l) 항목을 모두 다루어야 하며, `rqms.services.review.hold_process_review` 가 항목 누락을 거부한다. 검토 출력은 최고경영자에게 보고되고 경영검토 입력이 된다(9.3.2.1 b).

## 8. 운영 명령

```bash
python -m rqms clause 8.3.1
python -m rqms matrix process
python -m rqms check
```
