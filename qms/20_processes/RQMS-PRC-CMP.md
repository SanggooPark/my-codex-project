...
| --- | --- |
| 문서번호 | RQMS-PRC-CMP |
| 영문명 | Competence management process |
| 근거 조항 | ISO 22163:2023 7.2.1.1 |
| 구분 | 필수 프로세스 (Annex A.1) |
| 프로세스 오너 | 인사 관리자 (5.3.1 b, 5.3.2) |
| 상위 프로세스 | 없음 (최상위) |
| 하위 프로세스 | 없음 |
| 구현 모듈 | `rqms.services.competence` |
| 프로세스 검토 주기 | 12개월 (9.4) |

## 1. 목적 및 적용범위

본 프로세스는 ISO 22163:2023 7.2.1.1 의 요구사항을 충족하기 위해 역량 관리 프로세스를 수립·실행·유지한다.
...
| 조항 | 의무 | 출처 | 요구사항 |
| --- | --- | --- | --- |
| 7.1.2 | shall | ISO 9001:2015 | 인원 |
| 7.2 | shall | ISO 9001:2015 | 역량 |
| 7.2.1.1 | shall | ISO 22163 추가 | 역량 관리 프로세스(갭 식별·교육·이해 증거) |
| 7.2.1.2 | should | ISO 22163 추가 | 역량 매트릭스 및 교육자료 정기 검토 |
| 7.3 | shall | ISO 9001:2015 | 인식 |
...
| 통제 | 유형 | 중요도 | 내용 | 근거 조항 |
| --- | --- | --- | --- | --- |
| CTL-026 | invariant | major | 필요역량을 충족하지 않은 인원의 해당 업무 배정 금지 | 7.2, 7.2.1.1 |
...
| --- | --- | --- | --- | --- | --- | --- |
| PI-CMP 역량 갭 해소율 | PI | 해소된 역량 갭 수 / 식별된 역량 갭 수 x 100 | 90.0% | quarterly | 인사 관리자, 품질보증 책임자 | 인사 관리자 |
...
| --- | --- |
| 심사원 자격 기록 | `auditor` |
| 역량 요구 정의(역량 매트릭스) | `competence_requirement` |
| 역량 보유 기록 | `competence` |
| 프로젝트 인력계획서 | `project_plan` (조건: `plan_kind = 'hr'`) |
| 자원 계획 | `governance_record` (조건: `kind = 'resource_plan'`) |
| 교육 기록 | `process_training` |
